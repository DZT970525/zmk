...
#define TRACKPOINT_SCROLL_GAIN 0.80f

// --- Arrow key thresholds use the existing Kconfig settings ---
...
static int special_key_listener_cb(const zmk_event_t *eh) {
    ...
    if (ev->position == 21) {
        ...
        LOG_INF("Arrow position=20 %s", arrow_key_pressed ? "PRESSED" : "RELEASED");
    }

    // Scroll key (Space)
    if (ev->position == 50) {
        scroll_key_pressed = ev->state;
        LOG_INF("Scroll position=49 %s", scroll_key_pressed ? "PRESSED" : "RELEASED");
    }

    // ★ NEW: Slow key
    if (ev->position == 23) {
        slow_key_pressed = ev->state;
        LOG_INF("slow_key position=22 %s", slow_key_pressed ? "PRESSED" : "RELEASED");
    }
...
/* Match the A320's speed scaling and fractional scroll accumulation. */
static inline void process_cm5_scroll(const struct device *dev, struct trackpoint_data *data,
    ...
    data->scroll_residue_x += dx * scale * TRACKPOINT_SCROLL_GAIN;
    data->scroll_residue_y += dy * scale * TRACKPOINT_SCROLL_GAIN;
...
static void trackpoint_work_cb(struct k_work *work) {
    ...
    if (arrow_key_pressed) {
        ...
        /* The CM5 helper negates outputs; negate inputs to preserve this board's directions. */
        int16_t scroll_x = -(int16_t)dx * SCROLL_X_DIR;
        int16_t scroll_y = -(int16_t)dy * SCROLL_Y_DIR;
