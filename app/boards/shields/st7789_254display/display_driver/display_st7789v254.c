...
#define DT_DRV_COMPAT sitronix_st7789v254

#include "display_st7789v254.h"
...
LOG_MODULE_REGISTER(display_st7789v254);
...
static int st7789v_read(const struct device *dev, const uint16_t x, const uint16_t y,
...
static void *st7789v_get_framebuffer(const struct device *dev) { return NULL; }

static int st7789v_set_brightness(const struct device *dev, const uint8_t brightness) {
    return -ENOTSUP;
}

static int st7789v_set_contrast(const struct device *dev, const uint8_t contrast) {
    return -ENOTSUP;
}
