...
static void apply_capslock_state(bool new_capslock) {
    ...
    if (capslock_on) {
        /* Light immediately, then continue with the breathing animation. */
        brightness = BRT_MAX;
...
static int trackpad_led_hid_indicators_listener(const zmk_event_t *eh) {
    ...
    if (atomic_get(&indicator_ready)) {
        /* ZMK event listeners run in thread context, so update the PWM now.
         * Only the following breathing frames need the delayed work queue.
         */
        apply_capslock_state(new_capslock);
    }
