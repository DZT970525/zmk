...
#define ARROW_KEY_THRESHOLD 1
#define ARROW_RELEASE_DELAY_MS 40
...
static void arrow_repeat_work_handler(struct k_work *work) {
    ...
    if (!space_pressed || (!dx_acc && !dy_acc)) {
        k_work_schedule(&data->arrow_repeat_work, K_MSEC(ARROW_RELEASE_DELAY_MS));
        return;
    }
    ...
    if (dx > ARROW_KEY_THRESHOLD)
        trigger_right_arrow();
    else if (dx < -ARROW_KEY_THRESHOLD)
        trigger_left_arrow();

    if (dy > ARROW_KEY_THRESHOLD)
        trigger_down_arrow();
    else if (dy < -ARROW_KEY_THRESHOLD)
        trigger_up_arrow();
...
static int bbtrackball_init(const struct device *dev) {
    ...
    LOG_INF("Initializing BBtrackball (interrupt + workqueue + arrow key mode)...");
