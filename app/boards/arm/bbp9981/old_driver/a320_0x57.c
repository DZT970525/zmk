...
static int a320_scan_i2c_address(struct i2c_dt_spec *spec) {
    const uint8_t candidate_addrs[] = {0x3B, 0x37, 0x57};
    uint8_t dummy = 0;

    for (int i = 0; i < ARRAY_SIZE(candidate_addrs); i++) {

        uint16_t addr = candidate_addrs[i];

        int ret = i2c_write(spec->bus, &dummy, 0, addr);

        if (ret == 0) {
            LOG_INF("A320 detected at I2C address: 0x%02X", addr);
            spec->addr = addr;
            return 0;
        }
    }

    LOG_ERR("A320 not detected on any known address!");
    return -ENODEV;
}
...
static int a320_init(const struct device *dev) {
    const struct a320_dev_config *cfg_const = dev->config;
    struct a320_dev_config *cfg = (struct a320_dev_config *)cfg_const;
    ...
    /* 🔥 Real I2C scan happens here */
    if (a320_scan_i2c_address(&cfg->i2c) != 0) {
        return -ENODEV;
    }
