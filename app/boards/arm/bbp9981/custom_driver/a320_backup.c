...
static void arrow_repeat_work_handler(struct k_work *work) {
    ...
    /* Wait until window time elapsed */
    ...
    if (pin_state == 0) {
        ...
        if (data->read_motion(data->dev, &dx, &dy) == 0 && (dx || dy)) {
            ...
            /* Scroll mode when CapsLock is on */
                ...
                /* New gesture detection: if >60ms, reset residual */
                ...
                /* Dynamic scaling (keep original stepped style but smoother) */
                ...
                /* Accumulate residual (key improvement) */
                ...
                /* Keep fractional residual to avoid stair effect */
...
static int a320_init(const struct device *dev) {
    ...
    const uint8_t candidates[] = {0x3B, 0x57};

    for (int i = 0; i < ARRAY_SIZE(candidates); i++) {
        scan_spec.addr = candidates[i];

        /* Try reading one register to test ACK */
