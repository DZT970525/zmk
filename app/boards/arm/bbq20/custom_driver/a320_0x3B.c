...
/* HID Resolution Multiplier supports 16 sub-steps per normal wheel detent. */
#if IS_ENABLED(CONFIG_ZMK_POINTING_SMOOTH_SCROLLING)
#define A320_SCROLL_RESOLUTION 16.0f
#else
#define A320_SCROLL_RESOLUTION 1.0f
#endif
...
static int special_key_listener_cb(const zmk_event_t *eh) {
    ...
    if (ev->position == 25) {
        ...
        LOG_INF("Arrow position=25 %s", arrow_key_pressed ? "PRESSED" : "RELEASED");
    }
...
/* Smooth scrolling used by HackberryPi Q21 CM5. */
static inline void process_smooth_scroll(const struct device *dev, struct a320_data *data,
    ...
    data->scroll_residue_x += dx * scale * A320_SCROLL_RESOLUTION;
    data->scroll_residue_y += dy * scale * A320_SCROLL_RESOLUTION;
...
static void send_arrow_key(uint16_t key) {
    if (zmk_hid_keyboard_press(key) < 0)
        return;
...
static inline void process_arrow_axis(int16_t delta, int16_t *residue, uint16_t key_neg,
                                      uint16_t key_pos) {
    ...
    *residue += delta; // 替换掉 dir_mult
    ...
    if (arrow_ticks != 0) {
        ...
        /* Send a real keyboard arrow press/release, not a mouse button event. */
        send_arrow_key(key);
...
static void a320_work_cb(struct k_work *work) {
    ...
    bool scroll_mode = !arrow_key_pressed && (scroll_key_pressed || capslock);
    ...
    if (arrow_key_pressed) {
        ...
        /* Use the same physical rotation as normal pointer movement. */
        int16_t arrow_x = -dy;
        int16_t arrow_y = dx;
        ...
        int abs_dx = abs(arrow_x);
        int abs_dy = abs(arrow_y);
        ...
        process_arrow_axis(arrow_x, &data->arrow_residue_x, HID_USAGE_KEY_KEYBOARD_LEFTARROW,
                           HID_USAGE_KEY_KEYBOARD_RIGHTARROW);

        process_arrow_axis(arrow_y, &data->arrow_residue_y, HID_USAGE_KEY_KEYBOARD_UPARROW,
        ...
        process_smooth_scroll(dev, data, scroll_x, scroll_y, now);
    } else if (!capslock) {
