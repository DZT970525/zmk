...
static int hid_indicators_listener(const zmk_event_t *eh) {
    if (as_zmk_hid_indicators_changed(eh)) {
        ...
        current_indicators = ev->indicators; // 缓存最新状态
    }
...
static void polling_work_handler(struct k_work *work) {
    bool current_capslock = current_indicators & HID_INDICATORS_CAPS_LOCK;
...
ZMK_LISTENER(trackpad_led_hid_listener, hid_indicators_listener);
